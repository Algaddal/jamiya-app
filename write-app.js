    ...
    <div dir="rtl" style={{minHeight:"100vh",background:"linear-gradient(135deg,#0F1923,#1A2E28)",fontFamily:"'Tajawal',sans-serif",padding:24}}>
      <div style={{maxWidth:480,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20}}>
          {[{label:"المشاركون",val:round.pays.length+" عضو",icon:"👥"},{label:"دفعوا",val:paidCount+"/"+round.pays.length,icon:"✅"},{label:"صندوق الجولة",val:totalPot.toLocaleString()+" ر.س",icon:"💰"},{label:"تاريخ البدء",val:round.date||tod(),icon:"📅"}].map((s,i)=>(
...
function LoginScreen({onLogin}){
  ...
  async function handleLogin(){
    ...
    const{data,error}=await supabase.from("users").select("*").eq("phone",phone).eq("pin",pin).single();
...
export default function App(){
  ...
  async function loadAll(){
    setLoading(true);
    const[{data:members},{data:rounds},{data:hist},{data:settings},{data:pays}]=await Promise.all([
    ...
    setState({members:members||[],rounds:roundsWithPays,hist:hist||[],settings:sObj,pays:pObj});
    setLoading(false);
  }
  ...
  const curRoundNum=state.settings?.current_round||1;
  const curRound=state.rounds.find(r=>r.round_num===curRoundNum);
  const curParticipants=state.settings?.current_participants||[];
  const totalPot=curParticipants.reduce((s,mid)=>{const m=state.members.find(x=>x.id===mid);return s+(m?m.amt:0);},0);
  ...
  async function addMember(name,phone,amt){
    ...
    const allMembers=await supabase.from("members").select("id");
    const ids=(allMembers.data||[]).map(m=>m.id);
  ...
  async function confirmWin(winner,method){
    ...
    const payRecs=curParticipants.map(mid=>{const m=state.members.find(x=>x.id===mid);return{round_id:newRound.id,member_id:mid,member_name:m?m.name:"",paid:false,amt:m?m.amt:0};});
  ...
  return(
    <div dir="rtl" style={{minHeight:"100vh",background:bg,fontFamily:"'Tajawal',sans-serif",color:"#1A2E28",display:"flex"}}>
      <aside style={{width:230,background:G,display:"flex",flexDirection:"column",padding:"24px 0 16px",position:"fixed",right:0,top:0,bottom:0,zIndex:100}}>
          ...
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.5)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center"}}>
            <div style={{background:sf,borderRadius:20,padding:28,width:420,maxWidth:"95vw"}}>
              <div style={{textAlign:"center",marginBottom:20}}>
                ...
                <p style={{color:"#5A7A72",fontSize:13,marginTop:6}}>شارك الرابط مع المشاركين</p>
              </div>
...
function MembersTab({state,isAdmin,onAdd,onRemove}){
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}><div><h2 style={{fontSize:21,fontWeight:700,margin:0}}>الأعضاء</h2><p style={{fontSize:13,color:"#5A7A72",margin:"4px 0 0"}}>مرتبون حسب الالتزام بالدفع</p></div></div>
      {isAdmin&&(
        <div style={{background:sf,borderRadius:14,padding:"18px 22px",marginBottom:16,boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr auto",gap:10,alignItems:"end"}}>
            ...
            <button onClick={()=>{if(!name||!phone||!amt)return;onAdd(name,phone,amt);setName("");setPhone("");setAmt("");}} style={{padding:"9px 20px",borderRadius:8,background:G,color:"#fff",fontSize:14,fontWeight:700,border:"none",cursor:"pointer",fontFamily:"'Tajawal',sans-serif"}}>+ إضافة</button>
          </div>
        </div>)}
          ...
          return(
            <div key={m.id} style={{border:"1px solid "+bd,borderRadius:10,padding:"13px 15px",marginBottom:10}}>
              <div style={{display:"flex",alignItems:"center",gap:11,marginBottom:9}}>
                ...
                <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700}}>{m.name}</div><div style={{fontSize:12,color:"#5A7A72"}}>{m.phone} · {m.amt.toLocaleString()} ر.س/شهر</div></div>
                ...
                {isAdmin&&<button onClick={()=>{if(confirm("حذف "+m.name+"؟"))onRemove(m.id,m.name);}} style={{padding:"3px 10px",borderRadius:6,background:"#FCE4EC",color:"#880E4F",fontSize:11,fontWeight:700,border:"none",cursor:"pointer",fontFamily:"'Tajawal',sans-serif"}}>حذف</button>}
              </div>
...
function NewRoundTab({state,curRoundNum,curRound,curParticipants,totalPot,drawMode,setDrawMode,pendingWinner,setPendingWinner,spinning,spinDisplay,onStartDraw,onConfirm,onUpdateParticipants}){
  const eligible=curParticipants.filter(mid=>{const m=state.members.find(x=>x.id===mid);return m&&!m.won_round;});
  if(curRound)return(<div style={{background:sf,borderRadius:14,padding:28,boxShadow:"0 1px 4px rgba(0,0,0,.06)",maxWidth:500}}><div style={{background:"#E3F2FD",border:"1px solid #90CAF9",borderRadius:10,padding:"12px 16px",marginBottom:16,fontSize:13,color:"#1565C0"}}>الجولة #{curRound.round_num} جارية — الفائز: <strong>{curRound.winner_name}</strong></div></div>);
  return(
    <div>
      <div style={{marginBottom:20}}><h2 style={{fontSize:21,fontWeight:700,margin:0}}>إعداد الجولة #{curRoundNum}</h2><p style={{fontSize:13,color:"#5A7A72",margin:"4px 0 0"}}>حدد المشاركين ثم اختر الفائز</p></div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <div style={{background:sf,borderRadius:14,padding:"18px 22px",boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
          <div style={{fontSize:14,fontWeight:700,marginBottom:14,display:"flex",justifyContent:"space-between"}}>الخطوة 1 - المشاركون
          ...
          {state.members.map(m=>{
            const ci=state.members.indexOf(m)%6;
            const checked=curParticipants.includes(m.id);
            return(<div key={m.id} onClick={()=>{const n=checked?curParticipants.filter(x=>x!==m.id):[...curParticipants,m.id];onUpdateParticipants(n);}} style={{border:"2px solid "+(checked?G:bd),borderRadius:8,padding:"11px 14px",marginBottom:8,cursor:"pointer",display:"flex",alignItems:"center",gap:11,background:checked?GL:"transparent",transition:"all .18s"}}>
              ...
              <div style={{flex:1}}><div style={{fontWeight:700,fontSize:14}}>{m.name} {m.won_round?<span style={{fontSize:10,background:"#E3F2FD",color:"#1565C0",padding:"1px 7px",borderRadius:10,fontWeight:700}}>فاز #{m.won_round}</span>:null}</div><div style={{fontSize:12,color:"#5A7A72"}}>{m.amt.toLocaleString()} ر.س</div></div>
            </div>);})}
          <div style={{marginTop:14,paddingTop:14,borderTop:"1px solid "+bd,display:"flex",justifyContent:"space-between",fontSize:13}}>
            <span style={{color:"#5A7A72"}}>إجمالي:</span>
            <span style={{fontWeight:700,fontSize:16}}>{totalPot.toLocaleString()} ر.س ({curParticipants.length} مشارك)</span>
          </div>
        </div>
        <div>
          <div style={{background:sf,borderRadius:14,padding:"18px 22px",boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
            <div style={{fontSize:14,fontWeight:700,marginBottom:14}}>الخطوة 2 - اختيار الفائز</div>
            ...
            {drawMode==="manual"&&(
              <div>
                {eligible.map(mid=>{const m=state.members.find(x=>x.id===mid);if(!m)return null;const ci=state.members.indexOf(m)%6;const isSel=pendingWinner?.id===mid;return(<div key={mid} onClick={()=>setPendingWinner(m)} style={{border:"2px solid "+(isSel?G:bd),borderRadius:8,padding:"11px 14px",marginBottom:8,cursor:"pointer",display:"flex",alignItems:"center",gap:11,background:isSel?GL:"transparent"}}><div style={{width:36,height:36,borderRadius:"50%",background:AVBG[ci][0],color:AVBG[ci][1],display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:12}}>{ini(m.name)}</div><div style={{flex:1}}><div style={{fontWeight:700,fontSize:14}}>{m.name}</div><div style={{fontSize:12,color:"#5A7A72"}}>{m.amt.toLocaleString()} ر.س</div></div>{isSel&&<span style={{fontSize:18,color:G}}>✓</span>}</div>);})}
              </div>)}
            {pendingWinner&&(
              <div style={{background:GL,border:"2px solid "+GM,borderRadius:10,padding:16,margin:"12px 0"}}>
                <div style={{fontSize:11,fontWeight:700,color:G,marginBottom:4}}>الفائز</div>
                <div style={{fontSize:20,fontWeight:700,color:GD}}>{pendingWinner.name}</div>
                <div style={{fontSize:13,color:G,marginTop:4}}>المبلغ: {totalPot.toLocaleString()} ر.س</div>
              </div>)}
            {pendingWinner&&<button onClick={()=>onConfirm(pendingWinner,drawMode)} style={{width:"100%",padding:11,borderRadius:10,background:G,color:"#fff",fontSize:14,fontWeight:700,border:"none",cursor:"pointer",fontFamily:"'Tajawal',sans-serif",marginTop:8}}>✅ تأكيد الفوز وبدء الجولة التالية</button>}
          </div>
        </div>
      </div>
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <div><h2 style={{fontSize:21,fontWeight:700,margin:0}}>المدفوعات</h2></div>
        ...
        <div style={{background:sf,borderRadius:14,padding:"18px 22px",boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
          ...
          {round.pays.map(pay=>(
            <div key={pay.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:"1px solid "+bd}}>
              <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700}}>{pay.member_name}</div><div style={{fontSize:12,color:"#5A7A72"}}>{pay.amt.toLocaleString()} ر.س {pay.paid_date?"· "+pay.paid_date:""}</div></div>
...
function RoundsTab({state,onShare}){
  ...
  return(
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <div style={{background:sf,borderRadius:14,padding:"18px 22px",boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
          <div style={{fontSize:14,fontWeight:700,marginBottom:14}}>الجولات المكتملة</div>
            ...
            return(<div key={r.round_num} onClick={()=>setSel(r.round_num)} style={{border:"1px solid "+(r.round_num===sel?G:bd),borderRadius:10,padding:"13px 15px",marginBottom:9,cursor:"pointer",background:r.round_num===sel?GL:"transparent"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                <div style={{display:"flex",gap:6}}>
                  ...
                  <span style={{fontSize:11,padding:"2px 8px",borderRadius:20,background:GL,color:GD,fontWeight:700}}>{r.draw_method==="random"?"قرعة":"تراضي"}</span>
                </div>
              </div>
        ...
        {selRound&&(
          <div style={{background:sf,borderRadius:14,padding:"18px 22px",boxShadow:"0 4px 16px rgba(0,0,0,.08)",borderRight:"4px solid "+GM}}>
            ...
            <div style={{fontSize:12,color:"#5A7A72",marginBottom:16}}>{selRound.date} · الفائز: <strong style={{color:G}}>{selRound.winner_name}</strong></div>
            {selRound.pays.map(pay=>(
              <div key={pay.id} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 0",borderBottom:"1px solid "+bd,fontSize:13}}>
                ...
                <span style={{fontWeight:700}}>{pay.amt.toLocaleString()} ر.س</span>
              </div>))}
          </div>)}
      </div>
    </div>);
      ...
      <div style={{background:sf,borderRadius:14,padding:"18px 22px",boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
        ...
        {state.hist.map(h=>{const[ico,hbg,tc]=typeMap[h.type]||["📌","#F5F7F6","#5A7A72"];return(
          <div key={h.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:"1px solid "+bd}}>
            ...
            {h.amt>0&&<span style={{fontWeight:700,fontSize:14,color:tc}}>{h.amt.toLocaleString()} ر.س</span>}
          </div>);})}
      </div>
    </div>);
}
    ...
    <div>
      ...
      <div style={{background:sf,borderRadius:14,padding:"18px 22px",marginBottom:16,boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 120px 120px auto",gap:10,alignItems:"end"}}>
          ...
          <button onClick={()=>{if(!name||!phone||!pin)return;onAdd(name,phone,pin,role);setName("");setPhone("");setPin("");setRole("member");}} style={{padding:"9px 20px",borderRadius:8,background:G,color:"#fff",fontSize:14,fontWeight:700,border:"none",cursor:"pointer",fontFamily:"'Tajawal',sans-serif"}}>+ إضافة</button>
        </div>
      </div>
      <div style={{background:sf,borderRadius:14,padding:"18px 22px",boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
        {state.users?.map(u=>(
          <div key={u.id} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 0",borderBottom:"1px solid "+bd}}>
            ...
            <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700}}>{u.name} {u.id===currentUser.id?<span style={{fontSize:10,background:GL,color:GD,padding:"1px 7px",borderRadius:10,fontWeight:700}}>أنت</span>:null}</div><div style={{fontSize:12,color:"#5A7A72"}}>{u.phone}</div></div>
            ...
            {u.id!==currentUser.id&&<button onClick={()=>{if(confirm("حذف "+u.name+"؟"))onRemove(u.id);}} style={{padding:"4px 12px",borderRadius:6,background:"#FCE4EC",color:"#880E4F",fontSize:11,fontWeight:700,border:"none",cursor:"pointer",fontFamily:"'Tajawal',sans-serif"}}>حذف</button>}
          </div>))}
      </div>
    </div>);
}
